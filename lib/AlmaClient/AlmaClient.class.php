...
class AlmaClient {
  ...
  public function request($method, $params = array(), $check_status = TRUE) {
    ...
    // the HTTP client out.
    $request = drupal_http_request(url($this->base_url . $method, array('query' => $params)), array('ssl_version' => 'tlsv1','timeout' => 520.0));
  ...
  private static function filter_request_params($params) {
      ...
      foreach ($params as $key => $value) {
        if (in_array($key, $sensitive)) {
          ...
          // load can be grouped
          $value = drupal_substr(md5($value . self::$salt), 0, drupal_strlen($value));
        }
 ...
 /**
    ...
    */
   public function self_register($cpr, $pin_code, $name, $addr1, $addr2, $mail, $branch, $sms) {
    $params = array(
       'securityNumber' => $cpr,
       'borrCard' => $cpr,
       'pin' => $pin_code,
